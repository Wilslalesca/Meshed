...
export const RotatingHeader: React.FC = () => {
  ...
  useEffect(() => {
    ref.current = window.setInterval(() => setIdx(i => (i + 1) % IMAGES.length), INTERVAL_MS);
      ...
      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
        {IMAGES.map((_, i) => (
          <button
            ...
            className={`h-2 w-2 rounded-full ${i === idx ? 'bg-primary' : 'bg-foreground/30 hover:bg-foreground/60'} transition`}
          />
        ))}
      </div>
    </div>
